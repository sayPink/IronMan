<template>
  <div class="add-container" v-loading="loading">
    ...
    <mavon-editor :value="markdown" codeStyle="atom-one-dark" :toolbars="toolbars" @save="handleSubmit" @imgAdd="handleUploadImage"
...
import { upload, markDownMoveFile, getBlogClassNameList, getBlogTagList, editBlog, getBlogDetail } from "@/api/blog"
...
export default {
  ...
  async created() {
    ...
    let result = await getBlogDetail(this.$route.params)
    if(!result.data.code) return this.$message.error(result.data.msg)
    this.markdown = result.data.data[0].markdown
    this.html = result.data.data[0].html
    this.title = result.data.data[0].title
    this.cover = result.data.data[0].cover_url
    this.class_id = result.data.data[0].class_id
    this.tag_id =  result.data.data[0].tag_id.split(',').map(item => {
  ...
  methods: {
    ...
    handleSubmit(){
      ...
      this.$confirm('此操作将编辑博客, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'info'
      }).then(async()=>{
        let result = await editBlog(param)
